...
import { isUnsubscribeText } from "./automation-skip.js";

const HUMAN_WORDS = ["נציג", "יועצת", "לדבר עם בנאדם", "אדם אמיתי", "טלפון איתי"];

// זיהוי בקשת הסרה — חייב לעבוד גם אם המודל מפספס.
// מילה/ביטוי שלם בלבד: "הסר בבקשה" כן, "הסרטון לא נפתח" לא (זה היה באג אמיתי).
function isUnsubscribe(text) {
  return isUnsubscribeText(text);
}
...
export async function handleMessage(lead, history, incoming, askFn = askClaude, opts = {}) {
  ...
  const context = opts.student
    ? `⚠️ זהו/זוהי תלמיד/ה קיים/ת של המכללה (סטטוס ב-CRM: ${opts.student}). מצב שירות: ענה/י לפי הסעיף "מצב שירות לתלמידים קיימים". בלי שיווק, בלי שיעור מבוא, בלי שיחת התאמה. שם=${lead.name || "לא ידוע"}.${opts.studentInfo ? "\n" + opts.studentInfo + "\n(מותר לצטט מהנתונים האלה: קורס, מחזור, מרצה, תאריך פתיחה, יום ושעה. על יתרות/חובות — להפנות למזכירות.)" : ""}`
